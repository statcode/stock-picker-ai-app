...
const POLYGON_API_KEY = 'OidjByztI_Os8htOJ_PUBje408L5bAAp'
const OPENAI_API_KEY = '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'
...
async function fetchStockData() {
    ...
    try {
        ...
        //console.log('tickersArr: ', tickersArr);
        const stockData = await Promise.all(tickersArr.map(async (ticker) => {
            ...
            const url = `https://polygon-api-worker.jphilava.workers.dev/?ticker=${ticker}&startDate=${dates.startDate}&endDate=${dates.endDate}`
            ...
            if (response.ok && status === 200) {
                ...
                /*
               for (const key in data) { //looping data object to ensure request_id is there before deleting it for caching
                    if (data.hasOwnProperty(key)) { // Important to check for own properties
                ...
                throw new Error(`Worker Error: ${data.error}. Please check your ticker codes and try again.`)
            }
        }))

        console.log('fetchStockData stockData: ', stockData);
...
async function fetchReport(data) {
    ...
    console.log('will start fetching openai on server');
    try {
        const url = 'https://openai-api-worker.jphilava.workers.dev'
        const response = await fetch(url, {
            method: 'POST',
            headers: {
        ...
        console.log('response:', response);
            ...
            if(response.status === 200) {
                ...
                console.log('fetchReport response data: ', responseData);
        ...
        /*const openai = new OpenAI({
            ...
            const response = await openai.responses.create({
                model: 'gpt-4',
                temperature: 1.2, //how creative/random it is
                max_output_tokens: 160, //limit output to save credit/token cost, default infinite
                input: messages
        })*/
